...
export default class EvDbViewAddress extends EvDbStreamAddress {
  ...
  constructor(
    streamAddressOrType: EvDbStreamAddress | string,
    viewNameOrStreamId: string,
    viewName?: string,
  ) {
    if (
      streamAddressOrType instanceof EvDbStreamAddress &&
    ...
    if (
      typeof streamAddressOrType === "string" &&
      typeof viewNameOrStreamId === "string" &&
