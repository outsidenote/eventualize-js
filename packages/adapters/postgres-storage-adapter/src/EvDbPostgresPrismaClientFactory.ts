import { PrismaClient } from "@eventualize/relational-storage-adapter/EvDbPrismaClient";
...
export default class EvDbPostgresPrismaClientFactory {
  ...
  public static create(connectionString?: string): PrismaClient {
    ...
    if (!connStr) {
      throw new Error(
        "PostgreSQL connection string not provided and POSTGRES_CONNECTION env var is not set",
      );
    }
