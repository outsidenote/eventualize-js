...
> **Note on `TEvents` shape:** For `Extract<TEvents, { eventType: K }>` to work, `TEvents` must encode the event name as a literal. The current `withEvent<T>` accumulates `TEvents | (T & { readonly eventType: string })`, where `eventType` is widened to `string`. This needs to be tightened: `withEvent<T, TName extends string>(eventType: TName)` should accumulate `TEvents | (T & { readonly eventType: TName })` so the literal is preserved in the union. This is the only required change to `withEvent`'s signature.
