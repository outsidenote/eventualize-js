...
export async function setupDynamoDBTables(config: DynamoDBConfig): Promise<void> {
  ...
  const tableSchemas = [EVENTS_TABLE_SCHEMA, SNAPSHOTS_TABLE_SCHEMA, MESSAGES_TABLE_SCHEMA];
  ...
  for (const schema of tableSchemas) {
    try {
      ...
      if (!(error instanceof Error)) {
        throw new Error("Unknown error occurred while creating DynamoDB table");
      }
